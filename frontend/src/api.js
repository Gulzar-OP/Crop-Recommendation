...
export async function getCropDetails(cropName) {
  ...
  if (!response.ok) {
    throw new Error(data.message || "Unable to load crop details");
  }
...
export const getMe = () =>
  request("/api/v1/auth/me").then((result) => result.user);
export const recommendCrop = (values) =>
  request("/api/v1/recommendations", {
    method: "POST",
    body: JSON.stringify(values),
  });
export const getHistory = () => request("/api/v1/recommendations/history");
export const deleteHistory = (id) =>
  request(`/api/v1/recommendations/history/${id}`, { method: "DELETE" });
